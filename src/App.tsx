...
import { buildDataOutput, hexToString } from './utils';
...
function App() {
  ...
  const memonic =
    'age winner pulse repair country lift rally upon huge hold copy carpet';
  ...
  const rootAddress =
    'tb1p7udctqtfn2tw4lnuemslc0zn3tmv3y558ss66w7thq4rqqz94j5sw9c8as';
  ...
  const createCommitTx = async (
    ...
    console.log(
      'to use utxo have satoshi toUseUtxo.satoshi',
      toUseUtxo.satoshi
    );
  ...
  const createRevealTx = async (
    ...
    // get faucet
    const revealHash = base.reverseBuffer(
      base.fromHex(revealTxParams.commit_tx.getId())
    );
  ...
  const onInitMetaid = async () => {
    ...
    const initRevealHex = await createRevealTx(
      btcPrivateKey,
      rootAddress,
      initRevealTxParams
    );
  ...
  const onTestBuzz = async () => {
    ...
    const buzzRevealHex = await createRevealTx(
      btcPrivateKey,
      rootAddress,
      buzzRevealTxParams
    );
