...
import * as varuint from "./bip174/converter/varint";
import {
	Bip32Derivation,
	KeyValue,
	PartialSig,
	...
	TapKeySig,
	TapScriptSig,
} from "./bip174/interfaces";
import { checkForInput, checkForOutput } from "./bip174/utils";
import { fromOutputScript, toOutputScript } from "./address";
import { BufferReader, cloneBuffer, reverseBuffer } from "./bufferutils";
...
export class Psbt {
	...
	constructor(
		...
		const dpew = (obj: any, attr: string, enumerable: boolean, writable: boolean): any =>
	...
	verify(pubBuf: Buffer, witness: Buffer): boolean {
		...
		for (let i = 0; i < this.inputCount; i++) {
			if (isTaprootInput(this.data.inputs[i])) {
				const tweakKey = Buffer.from(taprootTweakPubkey(pubBuf.slice(1))[0]);
				const bufferReader = new BufferReader(witness);
				...
				const { hash } = getHashAndSighashType(
					this.data.inputs,
					i,
					pubBuf,
					this.__CACHE,
				...
				const bufferReader = new BufferReader(witness);
				const vector = bufferReader.readVector();
				const signature = bitcoinSignature.decode(vector[0]).signature;
				const witnessPubKey = vector[1];
				if (!pubBuf.equals(witnessPubKey)) {
					throw Error("pubKey error");
				}
				if (!signUtil.secp256k1.verifyWithNoRecovery(hash, signature, pubBuf)) {
					return false;
				}
			}
		}
...
class PsbtTransaction implements ITransaction {
	...
	addInput(input: any): void {
		if (
			...
			(!Buffer.isBuffer((input as any).hash) && typeof (input as any).hash !== "string") ||
