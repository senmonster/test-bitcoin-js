 ...
 */
import { Network } from "./bitcoinjs-lib/networks.js";
...
export function fromBase58Check(address: string): Base58CheckResult {
	const payload = base.fromBase58Check(address);
...
export function toBase58Check(hash: Buffer, version: number): string {
	...
	return base.toBase58Check(payload);
}

export function toBech32(data: Buffer, version: number, prefix: string): string {
	const words = base.bech32.toWords(data);
	words.unshift(version);

	return version === 0 ? base.bech32.encode(prefix, words) : base.bech32m.encode(prefix, words);
}
