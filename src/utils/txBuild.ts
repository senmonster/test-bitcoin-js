	...
	build(hashArray?: string[]): string {
		...
		for (let i = 0; i < eckeys.length; i++) {
			...
			if (this.bitcoinCash) {
				...
				hashType =
					bitcoin.Transaction.SIGHASH_ALL | bitcoin.Transaction.SIGHASH_BITCOINCASHBIP143;
				...
				if (addressType === "legacy") {
					...
					const prevOutScripts = this.inputs.map((o) =>
						bitcoin.address.toOutputScript(o.address, this.network)
					);
					const values = this.inputs.map((o) => o.value!);
					hash = this.tx.hashForWitnessV1(
						i,
						prevOutScripts,
						values,
...
export function signBtc(
	...
	network = network || bitcoin.networks.bitcoin;
	if (utxoTx.memo) {
		const buf = base.isHexString(utxoTx.memo)
			? base.fromHex(utxoTx.memo)
			: Buffer.from(base.toUtf8(utxoTx.memo));
		if (buf.length > 80) {
	...
	if (changeOnly) {
		const { inputAmount, outputAmount, virtualSize } = calculateTxSize(
			inputs,
			outputs,
			changeAddress,
			fakePrivateKey,
	...
	const changeAmount = inputAmount - outputAmount - virtualSize * feePerB;

	// sign process
	const txBuild = new TxBuild(2, network, false, hardware);
	for (let i = 0; i < inputs.length; i++) {
		const input = inputs[i] as utxoInput;
	...
	for (let i = 0; i < outputs.length; i++) {
		const output = outputs[i] as utxoOutput;
	...
	if (
		utxoTx.memo &&
		(utxoTx.memoPos == undefined ||
			utxoTx.memoPos < 0 ||
			utxoTx.memoPos > txBuild.outputs.length)
	) {
		txBuild.addOutput(
			"",
			0,
...
export function signBch(
	...
	const { inputAmount, outputAmount, virtualSize } = calculateBchTxSize(
		inputs,
		outputs,
		changeAddress,
		fakePrivateKey,
	...
	// sign process
	const txBuild = new TxBuild(2, network, true, hardware);
	for (let i = 0; i < inputs.length; i++) {
		const input = inputs[i] as utxoInput;
		txBuild.addInput(
			input.txId,
...
function calculateTxSize(
	...
	const preTxBuild = new TxBuild(2, network, false, hardware);
	let inputAmount = 0;
	for (let i = 0; i < inputs.length; i++) {
		const input = inputs[i] as utxoInput;
		const inputPrivKey = input.privateKey || privateKey;
		...
		if (memo && pos && preTxBuild.outputs.length == pos) {
			preTxBuild.addOutput(
				...
				base.toHex(
					bscript.compile(
						([OPS.OP_RETURN] as Stack).concat(
							base.isHexString(memo)
								? base.fromHex(memo)
								: Buffer.from(base.toUtf8(memo))
						)
					)
				)
			);
...
function calculateBchTxSize(
	...
	const preTxBuild = new TxBuild(2, network, true, hardware);
	let inputAmount = 0;
	for (let i = 0; i < inputs.length; i++) {
		const input = inputs[i] as utxoInput;
		preTxBuild.addInput(
...
export function ValidSignedTransaction(signedTx: string, utxoInputs?: [], network?: Network) {
	...
	for (let i = 0; i < transaction.ins.length; i++) {
		...
		if (addressType === "legacy") {
			...
			const prevOutScript = Buffer.of(
				...
				0xa9,
				0x14,
				...bcrypto.hash160(pubKey),
				0x88,
				0xac
			);
			const hash = transaction.hashForWitness(
				i,
				prevOutScript,
				utxo.value!,
				signatureData.hashType
			);
			if (!signUtil.secp256k1.verifyWithNoRecovery(hash, signatureData.signature, pubKey)) {
				throw Error("signature error");
			}
			...
			// taproot
			const signature = input.witness[0];
			const prevOutScripts = utxoInputs.map((o) =>
				bitcoin.address.toOutputScript((o as Input).address, network)
			);
			...
			const tweakedPubKey = taproot.taprootTweakPubkey(
				base.fromHex(utxo.publicKey!).slice(1)
			)[0];
			if (
				!schnorr.verify(base.toHex(signature), base.toHex(hash), base.toHex(tweakedPubKey))
			) {
				throw Error("signature error");
			}
		}
	...
	for (const in1 of transaction.ins) {
...
export function estimateBtcFee(utxoTx: utxoTx, network?: bitcoin.Network) {
	...
	const { virtualSize } = calculateTxSize(
		inputs,
		outputs,
		utxoTx.address,
		fakePrivateKey,
...
export function estimateBchFee(utxoTx: utxoTx, network?: bitcoin.Network) {
	...
	const { virtualSize } = calculateBchTxSize(
		inputs,
		outputs,
		utxoTx.address,
		fakePrivateKey,
